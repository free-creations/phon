...
public class GeneratePersonTable {
  ...
  private final double meanCrewSize = 10;
  private final double wantsToBeInCrewRatio = 0.5;

  private final int crewMemberCount;

  public GeneratePersonTable() throws FileNotFoundException, IOException {
    crewMemberCount = (int) ((maxCount * wantsToBeInCrewRatio) / meanCrewSize);
  ...
  private String crewAssignment() {
    if (hasCrew()) {
      ...
      int crew = ((int) (crewMemberCount * random)) + 1;
      return Integer.toString(crew);
    } else {
      return "NULL";
    }
  }
  ...
  private boolean hasCrew() {
    double random = Math.random();
    return (random < wantsToBeInCrewRatio);
  }
  ...
  private void makePrologue(PrintWriter output) {
    ...
    output.println("*/");
    makeCrewRecords(output);
  }

  private void makeCrewRecords(PrintWriter output) {
    for (int i = 0; i < crewMemberCount; i++) {
      output.println("INSERT INTO \"APP\".\"CREW\" VALUES("
              + String.format("%s,", i + 1) +//  "CREW" INTEGER NOT NULL GENERATED BY DEFAULT AS IDENTITY,
              String.format("'Crew[%s]'", i + 1) + //"NAME" VARCHAR(50)
              ");");
    }
    output.println("ALTER TABLE \"APP\".\"CREW\" "
            + "ALTER COLUMN \"CREW\" RESTART WITH " + (crewMemberCount + 1) + ";");
    output.println("");
    output.println("");
  }
