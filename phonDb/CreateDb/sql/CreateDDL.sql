...
ALTER TABLE "APP"."TEAM" DROP CONSTRAINT TEAM_FK1;
...
CREATE TABLE "APP"."CONTEST" (
  ...
  "PRIORITY" INTEGER -- the importance of this contest (1=must definitely be assigned,..,4=can be left vaccant)
);
...
CREATE TABLE "APP"."ALLOCATION" (
  "ALLOCATIONID" BIGINT NOT NULL, -- the primary key is maintained in table SEQUENCES
  ...
  "COMMITED" INTEGER, -- (boolean) if 1 this allocation has been commited to the person.
...
CREATE TABLE "APP"."JOB" (
  ...
  "PRIORITY" INTEGER, -- the importance of this job (1=must definitely be assigned,..,4=can be left vaccant)
...
CREATE TABLE "APP"."TEAM" (
  ...
  "NAME" VARCHAR(50), -- the name shown on printouts
  "PERSON" INTEGER -- an optional person responsible for this team
);
...
-- -----------------------------------------------------------------------------
-- Table SEQUENCES
-- Holds the sequence numbers for new primary key values.
-- -----------------------------------------------------------------------------
CREATE TABLE "APP"."SEQUENCES" (
  "SEQ_NAME" char(32) NOT NULL,
...
ALTER TABLE "APP"."SEQUENCES"
  ADD CONSTRAINT "SequPrimKey" PRIMARY KEY ("SEQ_NAME");
...
-- Table TEAM
ALTER TABLE "APP"."TEAM" 
  ADD CONSTRAINT "TEAM_FK1" FOREIGN KEY ("PERSON") 
  REFERENCES "APP"."PERSON" ("PERSONID"); 
...
-- -----------------------------------------------------------------------------
-- -----------------------------------------------------------------------------
--                      Populate the Sequence Table with starting values
-- -----------------------------------------------------------------------------
-- -----------------------------------------------------------------------------
INSERT INTO "APP"."SEQUENCES" ("SEQ_NAME", "SEQ_NUMBER")
  VALUES('SEQ_ALLOCATION', 100); -- leave 100 sequence numbers free for test